  ...
  this.getWorker_p = function(appSpec) {
    ...
    .then(function(listenPort) {
      ...
      return workerPromise.then(function(appWorker) {
        ...
        var shutdownDelay = appSpec.settings.shutdownDelay;
        if (typeof shutdownDelay == 'undefined')
          shutdownDelay = 5000;
        
        appWorkerHandle.on('release', function(refCount) {
          if (refCount === 0 && shutdownDelay < Infinity) {
        ...
        var connectPromise = connect_p('127.0.0.1', listenPort, 500,
          appSpec.settings.startupTimeout || 60000,
        ...
        connectPromise.then(
          ...
          function(err) {
            doReject(err);
            // App probably timed out--kill it
            try {
              appWorker.kill();
            } catch (killErr) {
              logger.error('Failed to kill process on port ' + listenPort + ': ' + killErr.message);
            }
          });
        exitPromise.fin(function() {
