...
function createLocation(locNode) {
  ...
  var startupTimeout = locNode.getValues('startup_timeout').seconds;
  if (startupTimeout) {
    if (startupTimeout > 0)
      settings.startupTimeout = startupTimeout * 1000;
    else
      settings.startupTimeout = Infinity;
  }
  var shutdownDelay = locNode.getValues('shutdown_delay').seconds;
  if (typeof(shutdownDelay == 'number')) {
    if (shutdownDelay < 0)
      settings.shutdownDelay = Infinity;
    else
      settings.shutdownDelay = shutdownDelay * 1000;
  }
